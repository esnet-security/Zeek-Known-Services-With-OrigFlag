...
export {
	...
	type ServicesInfo: record {
		...
		## A flag value to check if the orig_h is local or remote.
		is_local_orig:	bool	&log;
	};
	...
	global services: table[addr, port, bool] of set[string] &create_expire=1day;
...
event service_info_commit(info: ServicesInfo)
	...
	for ( s in tempservs )
		{
		local key = AddrPortServTriplet($host = info$host, $p = info$port_num, $serv = s, $orig = info$is_local_orig);
